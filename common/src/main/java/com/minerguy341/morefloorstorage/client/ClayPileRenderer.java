...
public class ClayPileRenderer implements BlockEntityRenderer<ClayPileBlockEntity>
{
    ...
    public void render(ClayPileBlockEntity pile, float partialTick, PoseStack pose, MultiBufferSource buffers, int packedLight, int packedOverlay)
    {
        ...
        final int seed = pile.getBlockPos().hashCode();
        ...
        for (int i = 0; i < stacks.size(); i++)
        {
            ...
            final float x = ClayPileLayout.offsetInLayer(layer, within % grid);
            final float z = ClayPileLayout.offsetInLayer(layer, within / grid);
            ...
            pose.translate(0.5f + x, ClayPileLayout.heightOf(layer) + i * 0.0002f, 0.5f + z);
